...
const EditChallenge = () => {
  ...
  useEffect(() => {
    const fetchChallengeDetails = async () => {
      try {
        ...
        setDifficulty(data.difficulty);
        setDifficulty(data.difficulty);
        setFlags((data.flags || []).map(f => ({ id: f.id, value: f.flag, points: f.points || 0 }))); // Adapt for challenge flags with points
      } catch (error) {
  ...
  const handleAddFlag = () => {
    setFlags([...flags, { id: flags.length > 0 ? Math.max(...flags.map(f => f.id)) + 1 : 1, value: '', points: 0 }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      flags: flags.map(f => ({ flag: f.value, points: f.points || 0 })), 
      docker_image: challengeDetails.docker_image,
      file_path: challengeDetails.file_path, 
      status: challengeDetails.status || 'upcoming', 
    };
                      ...
                      >
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setFlags(flags.map(flag => flag.id === f.id ? { ...flag, points: newPoints } : flag));
                          }}
